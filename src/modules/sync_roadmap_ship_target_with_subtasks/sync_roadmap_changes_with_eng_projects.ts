...
import {
  ...
  ENGINEERING,
  INFRA,
  KHULNASOFT_PRIVATE_ROADMAP, PIXEL_POINT, POSTGRES, PRODUCT,
  PRODUCT_DESIGN
...
const FIELDS_MAPPING = [
  {
    from: KHULNASOFT_PRIVATE_ROADMAP.targetShipMothFieldId,
    to: [
      {
  ...
  {
    from: KHULNASOFT_PRIVATE_ROADMAP.targetShipQuarterFieldId,
    to: [
      {
        projectId: CONSOLE.projectId,
...
export const handleRoadmapProjectItemChange = async (context: EmitterWebhookEvent<"projects_v2_item.edited"> & Omit<Context<EmitterWebhookEventName>, "id" | "name" | "payload">) => {
  ...
  logger("info", 'force sync field id', KHULNASOFT_PRIVATE_ROADMAP.forceSyncFieldId)

  const isForceSyncFieldChanged = context.payload.changes.field_value.field_node_id === KHULNASOFT_PRIVATE_ROADMAP.forceSyncFieldId;
...
export const handleRoadmapIssueChange = async (context: EmitterWebhookEvent<"issues.edited"> & Omit<Context<EmitterWebhookEventName>, "id" | "name" | "payload">) => {
  try {
    ...
    const childrenIssues = await issue.getChildrenIssues(context.octokit);
    if (!issue.connectedProjectItems[KHULNASOFT_PRIVATE_ROADMAP.projectId]) {
      return;
    }
    await syncFieldValuesWithSubtasks(
      context.octokit,
      issue.connectedProjectItems[KHULNASOFT_PRIVATE_ROADMAP.projectId],
      childrenIssues,
      FIELDS_MAPPING,
    )
  } catch(e) {
