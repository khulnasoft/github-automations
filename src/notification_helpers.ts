...
const isKhulnasoftRepo = (workflow_run: any) => (workflow_run.repository.name === 'khulnasoft')
...
export const getEnvChannelName = (workflow_run: { head_branch?: string }) => {
  ...
  } else if (isKhulnasoftRepo(workflow_run)) {
    if (workflow_run.head_branch == process.env.KHULNASOFT_STAGING_BRANCH_NAME) {
      return process.env.SLACK_DEPLOY_NOTIFICATIONS_CHANNEL
    }
    if (workflow_run.head_branch == process.env.KHULNASOFT_PRODUCTION_BRANCH_NAME) {
      return process.env.SLACK_DEPLOY_NOTIFICATIONS_CHANNEL_PRODUCTION
...
const getWorkflowEnv = (workflow_run: any) => {
  if (isConsoleRepo(workflow_run)) {
    ...
    if (workflow_run.head_branch == process.env.KHULNASOFT_STAGING_BRANCH_NAME) {
      return "*[ STAGING KHULNASOFT ]*";
    }
    if (workflow_run.head_branch == process.env.KHULNASOFT_PRODUCTION_BRANCH_NAME) {
      return "*[ PRODUCTION KHULNASOFT ]*";
    }
  }
...
export const workflowSucceedTemplate: TemplateFunc = (workflow_run: any) => {
  let component = 'cloud';
  if (isKhulnasoftRepo(workflow_run)) {
    component = 'storage'
  }
